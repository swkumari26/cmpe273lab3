...
import {fetchSharedContent} from '../actions/index';
...
class Shared extends Component {
        ...
        componentDidMount(){
            this.props.fetchSharedContent(this.props.user.id);
        }
...
if(user){username= user.lastName+','+user.firstName;}
  ...
  <br/><br/>
  <Header pageName="Shared Files" userName={username}/>
  <br/><br/>
    <Body files={files} tree={tree} user={user} sideBarOption="/shared/"/>
  </div>
  </div>
      )
      }
  ...
  function mapDispatchToProps(dispatch) {
    return {
        ...bindActionCreators({fetchSharedContent},dispatch)
    };
}
